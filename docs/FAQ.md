...
1. 补丁与当前版本不匹配
2. 补丁文件损坏
3. 资源文件格式错误（resources.arsc 必须�?STORED�?
4. 点击「清除补丁」回滚，然后重新生成
...
### Q: 支持哪些 Android 版本�?
**A:** 
- **最低版�?*: Android 5.0 (API 21)
- **最高版�?*: Android 14 (API 34)
- **推荐版本**: Android 7.0+ (API 24+)

**为什么不支持 Android 4.x�?*
- DEX 注入需�?`BaseDexClassLoader` 内部字段（API 21+�?
- 资源加载需要特定的 `AssetManager` API（API 21+�?
- SO 加载需�?ClassLoader 路径修改（API 21+�?

### Q: 在不�?Android 版本上有什么区别？
**A:**
- **Android 5.0-6.0 (API 21-23)**: 标准方案，完全支�?
- **Android 7.0-7.1 (API 24-25)**: 需要处理混合编译模�?
- **Android 8.0-9.0 (API 26-28)**: 需要处�?ResourcesImpl
- **Android 10+ (API 29+)**: 需要处理非 SDK 接口限制，部分功能受�?
...
if (NativePatchEngine.isAvailable()) {
    Log.i(TAG, "Native引擎可用，性能最�?);
} else {
    Log.i(TAG, "使用Java引擎，功能完�?);
}
```

### Q: Native 引擎�?Java 引擎有什么区别？
**A:**
- **Native 引擎**: 使用 C/C++ 实现，性能更高，速度�?2-3 �?
- **Java 引擎**: �?Java 实现，兼容性好，功能完�?
...
- 支持两种加密方式�?
  ...
  2. **密码加密**：使用自定义密码，客户端需要相同密码才能解�?
- 推荐使用 KeyStore 方案（更安全，无需管理密码�?
- 密码加密适合需要跨设备使用的场�?

### Q: 如何使用密码加密补丁�?
**A:** 使用 `encryptPatchWithPassword` �?`decryptPatchWithPassword` 方法�?
...
### Q: 如何在项目中集成�?
**A:** 参�?[使用文档](USAGE.md)，主要步骤：
1. 添加依赖模块
2. �?Application 中初始化
3. 实现补丁下载和应用逻辑
4. 添加错误处理和回滚机�?

### Q: 可以热更�?AndroidManifest.xml 吗？
**A:** 不可以，这是 Tinker 的限制：
...
### Q: 可以热更新四大组件吗�?
**A:**
- **Activity/Service/Receiver**: 可以修改已有组件的代�?
- **新增组件**: 不可以，需要在 Manifest 中声�?
- **删除组件**: 不可�?

### Q: 如何调试热更新？
**A:**
1. 查看 Logcat 日志（TAG: DexPatcher, ResourcePatcher, SoPatcher�?
2. 使用 Demo 应用的「系统信息」查看状�?
3. 使用 `printDexElements()` 打印 DEX 信息
4. 检查补丁文件是否正确生�?

### Q: 如何测试热更新？
**A:**
1. 使用提供的测�?APK（test-apks 目录�?
2. 修改代码或资源后生成�?APK
3. 生成补丁并应�?
4. 验证修改是否生效
5. 测试回滚功能
