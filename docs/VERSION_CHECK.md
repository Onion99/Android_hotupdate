...
从 1.3.7 版本开始，热更新系统会自动检测 APK 版本变化，并在覆盖安装新版本时自动清除旧补丁。
...
- 覆盖安装时，APK 文件被替换
- 但应用数据不会被清除（包括补丁文件）
- 启动时仍然加载 `/data/data/包名/files/update/` 中的旧补丁

## 解决方案

### 自动版本检测

在应用启动时（`Application.attachBaseContext()`），自动检测 APK 版本是否变化：

```java
// 获取当前 APK 版本
PackageInfo packageInfo = context.getPackageManager()
    .getPackageInfo(context.getPackageName(), 0);
long currentVersionCode = packageInfo.versionCode; // 或 getLongVersionCode()
...
3. **编译缓存**：`oat/` 目录
4. **配置信息**：SharedPreferences 中的补丁状态

### 版本信息保存

在应用补丁时，自动保存当前 APK 版本信息：
...
使用反射避免 D8/R8 编译器生成合成类导致的 `NoSuchMethodError`：
...
直接使用 `getLongVersionCode()` 会导致 D8/R8 编译器生成合成类（`$$ExternalSyntheticApiModelOutline0`），在补丁 DEX 中可能找不到这个合成类，导致 `NoSuchMethodError`。
